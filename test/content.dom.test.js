...
window.eval(fs.readFileSync(path.join(root, 'lib/filters.js'), 'utf8'));
window.eval(
  fs.readFileSync(path.join(root, 'content.js'), 'utf8').replace(
    /\nstart\(\);\s*$/,
    '\nglobalThis.__tbz = { applyFilters, extractCardData, state };'
  )
);
...
function run() {
  ...
  const tbz = window.eval('globalThis.__tbz');
  const apply = () => {
