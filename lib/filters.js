(function (global) {
  ...
  function normalizeSettings(raw) {
    ...
    if (isNonNegativeNumber(raw.deliveryFeeMin)) out.deliveryFeeMin = raw.deliveryFeeMin;
    ...
    if (raw.mode === 'hide' || raw.mode === 'dim') out.mode = raw.mode;
  ...
  function matchesFilters(card, settings) {
    ...
    if (s.deliveryFeeMin > 0 && !withinMin(card.deliveryFee, s.deliveryFeeMin)) return false;
  ...
  function hasActiveFilters(settings) {
    const s = normalizeSettings(settings);
    return s.freeDeliveryOnly || s.deliveryFeeMin > 0 || s.deliveryFeeMax > 0 || s.minOrderMax > 0;
  }
  ...
  function settingsSummary(settings) {
    ...
    if (s.deliveryFeeMin > 0) parts.push('bezorgkosten ≥ €' + formatPrice(s.deliveryFeeMin));
