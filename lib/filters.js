(function (global) {
  ...
  function normalizeSettings(raw) {
    ...
    const legacy = 'mode' in raw || 'deliveryFeeMin' in raw || 'freeDeliveryOnly' in raw;
    out.deliveryFeeMax = readLimit(raw.deliveryFeeMax, legacy);
    out.minOrderMax = readLimit(raw.minOrderMax, legacy);
    return out;
  }

  function readLimit(value, legacy) {
  ...
  function matchesFilters(card, settings) {
    ...
    if (s.deliveryFeeMax !== null && !withinMax(card.deliveryFee, s.deliveryFeeMax)) return false;
    if (s.minOrderMax !== null && !withinMax(card.minOrder, s.minOrderMax)) return false;
    return true;
  }
  ...
  function hasActiveFilters(settings) {
    const s = normalizeSettings(settings);
    return s.deliveryFeeMax !== null || s.minOrderMax !== null;
  }

  function settingsSummary(settings) {
    const s = normalizeSettings(settings);
    const parts = [];
    if (s.deliveryFeeMax !== null) {
      parts.push(
        s.deliveryFeeMax === 0
          ? 'gratis bezorging'
          : 'bezorgkosten ≤ €' + formatPrice(s.deliveryFeeMax)
      );
    }
    if (s.minOrderMax !== null) parts.push('min. bestelling ≤ €' + formatPrice(s.minOrderMax));
